 ...
 *
 * Best mode: BiRefNet-lite 512 (MIT), fp16/WebGPU via Transformers.js.
 * Fast/fallback: U2NETP (Apache-2.0), 4.36 MiB via ONNX Runtime Web/WASM.
 * Images never leave the browser; only model/runtime assets are downloaded.
 */
...
export function getBackgroundRemovalCapabilities() {
  ...
  return {
    webgpu,
    bestRecommended,
    recommendedMode: bestRecommended ? 'best' : 'fast',
    device,
  };
}
...
function modelProgressAdapter(onProgress, label, base, span) {
  return info => {
    ...
    if (p == null && Number.isFinite(info?.loaded) && Number.isFinite(info?.total) && info.total > 0) {
      p = info.loaded / info.total;
    }
    const file = info?.file || info?.name || '';
    const detail = file ? `${label}: ${String(file).split('/').pop()}` : label;
    report(onProgress, 'model', base + (p == null ? 0 : p * span), detail);
  };
}
...
async function loadBestBundle(onProgress) {
  ...
  bestBundlePromise = (async () => {
    ...
    const progressCallback = modelProgressAdapter(onProgress, 'Downloading best-quality model', 0.03, 0.42);
    const [model, processor] = await Promise.all([
      transformers.AutoModel.from_pretrained(BEST_MODEL, {
        dtype: 'fp16',
        device: 'webgpu',
        progress_callback: progressCallback,
      }),
      transformers.AutoProcessor.from_pretrained(BEST_MODEL, {
        progress_callback: progressCallback,
      }),
    ]);
  ...
  try {
    return await bestBundlePromise;
  } catch (error) {
    bestBundlePromise = null;
    throw error;
  }
}
...
async function loadOrt() {
  ...
  ortPromise = (async () => {
    ...
    // IrisFiles does not currently opt into COOP/COEP, so avoid spawning a
    // threaded WASM runtime that would require cross-origin isolation.
...
async function fetchCachedModel(onProgress) {
  let cached = null;
  try {
    const cache = await caches.open(FAST_MODEL_CACHE);
    cached = await cache.match(FAST_MODEL_CACHE_KEY);
  } catch {}
  if (cached) {
  ...
  try {
    const cache = await caches.open(FAST_MODEL_CACHE);
    await cache.put(FAST_MODEL_CACHE_KEY, new Response(merged, { headers: { 'Content-Type': 'application/octet-stream' } }));
  } catch {}
...
async function loadFastSession(onProgress) {
  ...
  fastSessionPromise = (async () => {
    const [ort, modelBytes] = await Promise.all([loadOrt(), fetchCachedModel(onProgress)]);
  ...
  try {
    return await fastSessionPromise;
  } catch (error) {
    fastSessionPromise = null;
    throw error;
  }
}
...
async function runBest(file, onProgress) {
  ...
  const dims = probabilities.dims;
  const width = dims[dims.length - 1];
  const height = dims[dims.length - 2];
  const data = probabilities.data;
  const values = new Float32Array(width * height);
  for (let i = 0; i < values.length; i++) values[i] = Number(data[i]);
  return { mask: makeMaskCanvas(values, width, height), model: 'BiRefNet-lite 512', quality: 'best' };
}
...
function preprocessFast(image) {
  ...
  for (let i = 0; i < pixels.length; i += 4) {
    maxValue = Math.max(maxValue, pixels[i] / 255, pixels[i + 1] / 255, pixels[i + 2] / 255);
  }
...
async function runFast(decoded, onProgress) {
  ...
  const tensorData = preprocessFast(decoded.image);
  ...
  const values = new Float32Array(320 * 320);
  for (let i = 0; i < values.length; i++) values[i] = (Number(raw[i]) - min) / span;
  return { mask: makeMaskCanvas(values, 320, 320), model: 'U²-Netp', quality: 'fast' };
}
...
function guidedRefine(sourceImageData, maskImageData, width, height, strength) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (original <= 0.015) q = 0;
      ...
      const low = strength === 'soft' ? 0.035 : 0.055;
      const high = strength === 'soft' ? 0.965 : 0.945;
...
async function refineMask(decoded, rawMask, strength, onProgress) {
  ...
  sourceCanvas.width = maskCanvas.width = size.width;
  sourceCanvas.height = maskCanvas.height = size.height;
  const sourceCtx = sourceCanvas.getContext('2d', { willReadFrequently: true });
  const maskCtx = maskCanvas.getContext('2d', { willReadFrequently: true });
  sourceCtx.drawImage(decoded.image, 0, 0, size.width, size.height);
  maskCtx.imageSmoothingEnabled = true;
  maskCtx.imageSmoothingQuality = 'high';
  maskCtx.drawImage(rawMask, 0, 0, size.width, size.height);

  const refined = guidedRefine(
    sourceCtx.getImageData(0, 0, size.width, size.height),
    maskCtx.getImageData(0, 0, size.width, size.height),
    size.width,
    size.height,
    strength,
  );
  maskCtx.putImageData(refined, 0, 0);
  sourceCanvas.width = sourceCanvas.height = 1;
  rawMask.width = rawMask.height = 1;
  return maskCanvas;
}

export async function generateBackgroundMask(file, options = {}, onProgress = () => {}) {
  const requested = options.quality || 'auto';
  const edgeRefinement = options.edgeRefinement || 'normal';
  ...
  try {
    ...
    let chosen = requested === 'auto' ? capabilities.recommendedMode : requested;
    let result;
    let fallbackReason = null;

    if (chosen === 'best') {
      try {
        result = await runBest(file, onProgress);
      } catch (error) {
        ...
        result = await runFast(decoded, onProgress);
        chosen = 'fast';
      }
    } else {
      result = await runFast(decoded, onProgress);
...
async function canvasToBlob(canvas, mime, quality) {
  return new Promise((resolve, reject) => canvas.toBlob(
    blob => blob ? resolve(blob) : reject(new Error('Could not encode output image.')),
    mime,
    quality,
  ));
}
...
export async function renderBackgroundResult(file, maskCanvas, options = {}) {
  ...
  try {
    ...
    const outputType = options.outputType || (background === 'transparent' ? 'png' : 'jpg');
    ...
    if (crop) {
      ...
      for (let y = 0; y < maskCanvas.height; y++) {
        for (let x = 0; x < maskCanvas.width; x++) {
          if (data[(y * maskCanvas.width + x) * 4] < 16) continue;
      ...
      if (maxX >= minX && maxY >= minY) {
        ...
        cropBox = {
          x: Math.floor(minX / maskCanvas.width * decoded.width),
          y: Math.floor(minY / maskCanvas.height * decoded.height),
          width: Math.max(1, Math.ceil((maxX - minX + 1) / maskCanvas.width * decoded.width)),
          height: Math.max(1, Math.ceil((maxY - minY + 1) / maskCanvas.height * decoded.height)),
        };
      }
    }

    const canvas = document.createElement('canvas');
    canvas.width = cropBox.width; canvas.height = cropBox.height;
    const ctx = canvas.getContext('2d');
    if (background !== 'transparent') {
      ctx.fillStyle = background === 'white' ? '#ffffff' : background === 'black' ? '#000000' : background;
      ctx.fillRect(0, 0, canvas.width, canvas.height);
    }

    const sourceCanvas = document.createElement('canvas');
    sourceCanvas.width = decoded.width; sourceCanvas.height = decoded.height;
    const sourceCtx = sourceCanvas.getContext('2d');
    sourceCtx.drawImage(decoded.image, 0, 0);
    sourceCtx.globalCompositeOperation = 'destination-in';
    sourceCtx.imageSmoothingEnabled = true;
    sourceCtx.imageSmoothingQuality = 'high';
    sourceCtx.drawImage(maskCanvas, 0, 0, decoded.width, decoded.height);
...
export function cloneMaskCanvas(maskCanvas) {
  ...
  clone.width = maskCanvas.width; clone.height = maskCanvas.height;
...
export function paintMaskStroke(maskCanvas, points, options = {}) {
  ...
  const mode = options.mode === 'erase' ? 'erase' : 'restore';
  const radius = Math.max(2, options.radius || 24);
  const hardness = Math.max(0.1, Math.min(1, options.hardness ?? 0.8));
  ...
  ctx.lineWidth = radius * 2 * Math.max(0.35, hardness);
  if (points.length === 1) {
    ctx.beginPath(); ctx.arc(points[0].x, points[0].y, radius * Math.max(0.35, hardness), 0, Math.PI * 2); ctx.fill();
  } else {
  ...
  if (hardness < 0.95) {
    ctx.globalAlpha = 0.18 + (1 - hardness) * 0.18;
