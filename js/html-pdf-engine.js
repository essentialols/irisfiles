...
export async function htmlFileToPdf(file,onProgress=()=>{}){
  ...
  const iframe=document.createElement('iframe');iframe.setAttribute('sandbox','allow-same-origin');iframe.setAttribute('aria-hidden','true');Object.assign(iframe.style,{position:'fixed',left:'-100000px',top:'0',width:'1100px',height:'900px',border:'0',visibility:'hidden'});document.body.appendChild(iframe);
  try{
    ...
    onProgress(35,'Rendering HTML...');const canvas=await html2canvas(root,{backgroundColor:'#ffffff',scale:1,useCORS:false,allowTaint:false,logging:false,width,height,windowWidth:width,windowHeight:height});
