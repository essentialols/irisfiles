...
export async function renderPdfThumbnails(file,onProgress=()=>{}){
  ...
  for(let n=1;n<=doc.numPages;n++){
    ...
    pages.push({index:n-1,pageNum:n,sourceRotation:((page.rotate||0)%360+360)%360,preview:canvas.toDataURL('image/jpeg',.7)});canvas.width=canvas.height=1;onProgress(Math.round(n/doc.numPages*100));
  }return pages;
}
